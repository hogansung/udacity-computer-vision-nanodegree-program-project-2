...
def get_loader(
    ...
    if mode == "train":
        ...
        data_loader = data.DataLoader(
            dataset=dataset,
            batch_size=dataset.batch_size,  # this is set to 1
            shuffle=True,
            num_workers=num_workers,
        )
...
class CoCoDataset(data.Dataset):
    def __init__(
        ...
        if self.mode == "train":
            ...
            self.annotation_indices = list(self.coco.anns.keys())
            print("Obtaining caption lengths...")
            all_tokens = [
                nltk.tokenize.word_tokenize(
                    str(
                        self.coco.anns[self.annotation_indices[annotation_index]][
                            "caption"
                        ]
                    ).lower()
                )
                for annotation_index in tqdm(np.arange(len(self.annotation_indices)))
            ]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ann_id = self.annotation_indices[index]
            ...
            caption = torch.Tensor(caption).type(torch.LongTensor)
    ...
    def get_train_indices(self):
        ...
        all_indices = np.where(
            [
                self.caption_lengths[i] == sampled_caption_length
